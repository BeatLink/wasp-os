...
This fork starts from [wasp-os](https://github.com/wasp-os/wasp-os) and aims to
offer everything that both wasp-os and
[InfiniTime](https://github.com/InfiniTimeOrg/InfiniTime) can do, with every
feature meant to go back to wasp-os upstream one pull request at a time. This
document is the master inventory of those features. Every item carries a status
checkbox so progress can be tracked here.

Sources: InfiniTime 1.16.0 (`InfiniTimeOrg/InfiniTime` main, September 2026)
and wasp-os v0.4-189 (`wasp-os/wasp-os` master, August 2026) plus this fork's
own work.
...
How to read the Status column: `[x]` done, `[ ]` not started, `[~]` in
...
The toolchain comes from the nix flake, so nothing has to be installed by hand:
...
A headless simulator run also needs `SDL_VIDEODRIVER=dummy`. The firmware is
licensed under the LGPL v3 with an exception for MicroPython applications; see
[COPYING](COPYING) and [COPYING.LGPL](COPYING.LGPL).

---

## 1. Platform baseline

| Area | InfiniTime | wasp-os | Status |
|---|---|---|---|
...
| Capability | InfiniTime | wasp-os | Status |
|---|---|---|---|
...
| Face | InfiniTime | wasp-os | Status |
|---|---|---|---|
...
| Item | InfiniTime | wasp-os | Status |
|---|---|---|---|
...
| Setting | InfiniTime | wasp-os | Status |
|---|---|---|---|
...
this roadmap because this fork inherits them.
...
- [x] MicroPython is old (wasp-os pins a 1.12 base); this fork tracks 1.29, so the `-Wno-error` workarounds for GCC 13 and newer are gone.
...
3. **Standard GATT services.** Device Information, Battery, Current Time, Heart Rate, Immediate Alert, Motion, Music, Weather with forecast, Navigation, so InfiniTime companion apps work with wasp-os.
