...
How watch apps are packaged, installed over Bluetooth, enabled and configured from a
...
- [x] Try it on real hardware. The manager answers, a package built by `mkpkg.py` installs over
      base64 with agreeing checksums, `enable` persists, `icon_of` returns the icon, and
      `import pkg.flashlight.app` instantiates the class. What the run taught is below.

The prototype has to be uploaded as `/flash/pkgmgr.mpy`, not as source. A watch sitting at its
clock face has about 5.5 KB of heap free, and compiling the 7.7 KB module on the device needs far
more than that. The 13 KB seen after a soft reboot is not headroom: that is a watch whose apps
have not registered yet, and it drops back to 5.5 KB once they have. The compiled module is 2.6 KB
and imports in about 4 KB. That is most of what a booted watch has, so importing the manager from
`/flash/main.py` would leave nothing for the system; the prototype was driven from the REPL
instead. This raises the manager from a nice-to-freeze to a must-freeze, which the firmware list
below already covers.

`abi()` on the watch reports bytecode 6, architecture 7 and `raw: false`, so base64 really is the
only mode until `MICROPY_PY_SYS_STDIO_BUFFER` is enabled, exactly as this design assumed.

The working directory on the watch is `/flash`, so the manager's relative `pkg/` paths land in
`/flash/pkg/` without it needing to know the mount point.
