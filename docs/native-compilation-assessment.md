...
on the watch. Written against NeoTime at commit e806ad7, September 2026, with the firmware
...
MicroPython is not an interpreter bolted onto plain source files. Everything in a NeoTime
...
- **Buys:** roughly a doubling on loop-heavy pure-Python code. Much less on the UI code that
  dominates NeoTime, because native code still uses boxed objects, dictionary lookups for
...
Master vendors MicroPython 1.16, commit `b349588876` from November 2021. This work is already
done on the unmerged `worktree-micropython-upgrade` branch, which moves to 1.29 and repoints
the vendored submodule at a NeoTime fork.
...
- **Costs:** the branch is untested on hardware. Until it has run on a watch, none of the
  figures here can be confirmed against the new runtime.
- **Verdict:** finish and land it before spending effort anywhere else in this document. Every
  other measurement should be taken against the runtime NeoTime is actually going to ship.
